...
xc_distances <-
  select(odb_first_pings, registration2, long, lat)  %>%
...
odb_live %>%
  ...
  mutate(
    telegram_message_glidernet = glue(
      "Aircraft {registration.x} recently took off and is currently at {round(alt_feet.x,0)}' AMSL doing {ground_speed_kph.y}kph https://live.glidernet.org/#c={lat.y},{long.y}&z=13&s=1&w=0&p=2"
    ),
    telegram_message = glue(
      "{aircraft_type_name.x} '{registration.x}' recently took off from {nearest_site_name.y}. Altitude {round(alt_feet.x,0)}' AMSL with ground speed {ground_speed_kph.x}kph https://www.gliderradar.com/center/{lat.x},{long.x}/zoom/13/time/15/maptype/terrain"
    )
  ) %>%
...
if (nrow(odb_new_pings) > 0) {
  ...
  odb_new_messages %>%
    mutate(
      telegram_message = glue(
        "{aircraft_type_name} '{registration}' first location ping received at {nearest_site_name}. Altitude {round(alt_feet,0)}' AMSL with ground speed {ground_speed_kph}kph https://www.gliderradar.com/center/{lat},{long}/zoom/13/time/15/maptype/terrain"
      )
    ) %>%
...
xc_distances %>%
  ...
  mutate(
    telegram_message = glue(
      "{aircraft_type_name} '{registration}' is on XC from {nearest_site_name.y}, passing {location_name_live} at {distance_live}km. Altitude {round(alt_feet,0)}' AMSL with ground speed {ground_speed_kph}kph https://www.gliderradar.com/center/{lat},{long}/zoom/13/time/15/maptype/terrain"
    )
  ) %>%
