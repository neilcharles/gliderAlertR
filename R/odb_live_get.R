...
odb_live_get <- function(){
  ...
  if (file.exists("odb_first_pings.RDS")) {
    ...
    odb_first_pings <- odb_live[0, ] |>
      dplyr::mutate(nearest_site_name=NA)
  }
  ...
  #Calculate current nearest site
  odb_live <- odb_live |>
  ...
  #Join to first pings to get the origin site name
  odb_live <- odb_live |>
    dplyr::left_join(
      dplyr::select(
        odb_first_pings,
        ...
        dplyr::rename(origin_site_name = nearest_site_name),
      by = "registration2"
    ) |>
    dplyr::mutate(origin_site_name = ifelse(is.na(origin_site_name),
                                            nearest_site_name,
                                            origin_site_name))
  ...
  #XC Distances
  telegram_xc <- xc_distances_report |>
    dplyr::left_join(
    dplyr::select(odb_live,
           registration2,
           aircraft_type_name,
           alt_feet,
           ground_speed_kph),
    ...
    purrr::walk2(
      .x = telegram_xc$telegram_message,
      .y = telegram_xc$telegram_group_id,
      .f = ~ send_telegram(.x, .y)
    )
